...
const DROPBOX_APP_FOLDER_PATH = '/Apps/Typomatique';
...
function resolveConfiguredAppKey(explicitAppKey = '') {
  ...
  if (canUseDom()) {
    const windowValue = normalizeAppKey(window.TYPOMATIQUE_DROPBOX_APP_KEY || window.TYPEWRITER_DROPBOX_APP_KEY);
...
export async function completeDropboxAuthRedirectOnCallbackPage(options = {}) {
  ...
  const keyPrefix = prefix || (typeof options.storageKey === 'string' ? options.storageKey : 'typomatique');
